...
# 总原则
- 七张表维护当前事实状态，不是关键词出现日志。每轮正文完成后按0→1→2→3→4→5→6逐表检查。
...
- 表5“人物发展表”负责保存NPC最新发展锚点：姓名、当前修为、主要能力、当前地点、年龄或最后确认时间、当前重要状态、主要目标/重要事项。同一NPC只保留一行。
...
- 剧情/回答主体写完后，先立即输出一个完整<tableEdit>：有表格变化时写入所有必要的insertRow/updateRow/deleteRow；没有任何需要记录的变化时输出<tableEdit><!-- NO_CHANGE --></tableEdit>。随后再写参考行动、选项、伊依留言等回复尾部内容。`,
    ...
    refresh_system_message_template: `你是世界状态表格整理助手。只根据已确认事实维护现有七张表。优先更新已有行，不写流水账，不猜测未知。人物主表保存NPC识别信息，人物发展表保存最新发展锚点；历史表只保存影响未来推演的重要节点，不自行模拟NPC离线发展。只输出<tableEdit>。`,
    refresh_user_message_template: `<聊天记录>\n$1\n</聊天记录>\n<当前表格>\n$0\n</当前表格>\n<表头信息>\n$2\n</表头信息>\n按0当前状态→1角色状态→2背包→3任务约定→4人物主表→5人物发展表→6历史事件检查。同一对象已有行优先update；人物主表负责识别，人物发展表负责最新锚点，重要节点才写历史；不猜测未知。函数放在<tableEdit><!-- ... --></tableEdit>中。`,
