    ...
    input.addEventListener('change', async (event) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                let keyListHTML = '<ul>';
                firstLevelKeys.forEach(key => { keyListHTML += `<li>${key}</li>`; });
                keyListHTML += '</ul>';
                ...
                for (let key in importedData) USER.tableBaseSetting[key] = importedData[key];
...
function InitBinging() {
    ...
    $(document).on('click', '#table_test_api_button',async () => {
        ...
        await handleApiTestRequest(apiUrl, encryptedApiKeys, modelName);
    });
