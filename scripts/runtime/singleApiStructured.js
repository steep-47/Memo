...
function singleApiActive(){const settings=USER?.tableBaseSetting;return !independentEnabled()&&settings?.isExtensionAble!==false&&settings?.isAiWriteTable!==false&&settings?.step_by_step!==true;}
...
function saveCompleteSnapshot(piece){if(!piece)return;for(const sheet of BASE.getChatSheets?.()??[])sheet?.save?.(piece,true);}
