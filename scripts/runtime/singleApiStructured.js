...
function forceCustomJsonObject(generateData){
    const current=String(generateData?.custom_include_body??'').trim();
    if(/(?:^|\n)\s*response_format\s*:/i.test(current)||/"response_format"\s*:/i.test(current))return false;
    generateData.custom_include_body=`${current}${current?'\n':''}response_format:\n  type: json_object`;
    return true;
}
