...
function extractCalls(text) {
    ...
    while (i < source.length) {
        ...
        if (depth !== 0) return { ok:false, error:`${name} 缺少右括号`, calls:[] };
        const argsText = source.slice(match.lastIndex, cursor);
        let args;
        try { args = JSON5.parse(`[${argsText}]`); }
        catch (error) { return { ok:false, error:`${name} 参数无法解析：${error?.message || error}`, calls:[] }; }
...
function validateAction(call) {
    ...
    if (call.name === 'insertRow') {
        ...
        for (const key of Object.keys(data)) {
            ...
            if (!Number.isInteger(col) || col < 0 || col >= columnCount) return { ok:false, error:`insertRow(${index}) 列${key}越界` };
        }
    ...
    for (const key of Object.keys(data)) {
        const col = Number(key);
        if (!Number.isInteger(col) || col < 0 || col >= columnCount) return { ok:false, error:`updateRow(${index},${rowIndex}) 列${key}越界` };
    }
...
function ordered(actions) {
    ...
    return [...actions].sort((a,b) => {
        if (a.type === 'delete' && b.type === 'delete' && a.tableIndex === b.tableIndex) return b.rowIndex - a.rowIndex;
...
export function parseMemoTableEdit(raw) {
    ...
    const parsed = extractCalls(joined);
    if (!parsed.ok) return { ok:false, noChange:false, actions:[], error:parsed.error };
    if (!parsed.calls.length) {
        if (noChange) return { ok:true, noChange:true, actions:[], error:'' };
        return { ok:false, noChange:false, actions:[], error:'没有可执行的insertRow/updateRow/deleteRow，也不是NO_CHANGE' };
    }
    const actions = [];
    ...
    return { ok:true, noChange:false, actions:ordered(actions), error:'' };
}
