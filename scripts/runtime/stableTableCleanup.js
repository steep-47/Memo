...
async function runStableCleanup(){if(running)return EDITOR.warning('表格整理正在进行中');running=true;try{ensureSevenTableWorld();repairMissingColumnsBeforeCleanup();const reference=BASE.getLastSheetsPiece();const piece=reference?.piece;if(!piece?.hash_sheets)return EDITOR.error('表格整理失败：没有找到可整理的表格记录');const tableText=getTablePromptByPiece(piece);if(!String(tableText||'').trim())return EDITOR.error('表格整理失败：当前表格内容无法读取');const recentChat=await buildRecentChat();const userPrompt=`<当前七表>\n${tableText}\n</当前七表>\n<最近聊天>\n${recentChat}\n</最近聊天>\n\n请按0→1→2→3→4人物主表→5人物发展表→6历史事件逐表检查重复、过期、错位和应合并的数据。人物发展表的“年龄”和“最后确认时间”必须分别维护。按现有rowIndex生成必要的tableEdit操作，不要为了“更完整”编造未知信息。`;const useMainApi=USER.tableBaseSetting.use_main_api!==false;let rawContent;try{rawContent=useMainApi?await handleMainAPIRequest(SYSTEM_PROMPT,userPrompt):await handleCustomAPIRequest(SYSTEM_PROMPT,userPrompt);}catch(error){return EDITOR.error('表格整理API请求失败',error?.message||String(error),error);}if(rawContent==='suspended')return EDITOR.info('表格整理已取消');if(typeof rawContent!=='string'||!rawContent.trim())return EDITOR.error('表格整理失败：API返回为空，原表未修改');const{matches}=getTableEditTag(rawContent);if(!matches||matches.length===0){const tail=rawContent.replace(/\s+/g,' ').trim().slice(-260);console.warn('[Memo][table-cleanup] AI未返回tableEdit:',rawContent);return EDITOR.error(`表格整理失败：模型未返回tableEdit，原表未修改｜末尾：${tail}`);}const joined=matches.join('\n');if(/NO_CHANGE/i.test(joined)&&!/(?:insertRow|updateRow|deleteRow)\s*\(/.test(joined))return EDITOR.success('表格检查完成：当前无需整理');if(USER.tableBaseSetting.bool_silent_refresh!==true){const preview=`<div style="max-height:55vh;overflow:auto"><p>AI准备执行以下表格整理操作：</p><pre style="white-space:pre-wrap">${escapeHtml(joined)}</pre><p>确认后才会修改当前表格。</p></div>`;const confirmed=await EDITOR.callGenericPopup(preview,EDITOR.POPUP_TYPE.CONFIRM,'表格整理确认',{okButton:'执行',cancelButton:'取消'});if(!confirmed)return EDITOR.info('表格整理已取消，原表未修改');}const ok=executeTableEditActions(matches,piece);if(!ok)return EDITOR.error('表格整理执行失败，未能保存操作');USER.saveChat();BASE.refreshContextView();updateSystemMessageTableStatus();EDITOR.success('表格整理完成');}catch(error){console.error('[Memo][table-cleanup] 整理失败:',error);EDITOR.error('表格整理失败',error?.message||String(error),error);}finally{running=false;}}
function install(){if(window[INSTALL_FLAG])return;window[INSTALL_FLAG]=true;console.log('[Memo] 七表稳定tableEdit整理器已加载');}
