...
const WORLD_MEMORY_HEADERS = {
    ...
    '人物主表': ['姓名','性别','别名/称呼','身份/所属','外貌特征','性格','与玩家关系','重要信息'],
    '人物发展表': ['姓名','修为','主要能力','当前地点','年龄/最后确认时间','当前状态','主要目标/重要事项'],
    '历史事件表': ['时间','地点','涉及人物','事件','结果'],
};

const HEADER_ALIASES = {
    '人物主表': {'别名':'别名/称呼','称呼':'别名/称呼','所属势力':'身份/所属'},
    '人物发展表': {
        '当前所在地点':'当前地点','所在地点':'当前地点','所在地':'当前地点',
        '最后确认时间':'年龄/最后确认时间','年龄/确认时间':'年龄/最后确认时间',
        '能力':'主要能力','当前目标':'主要目标/重要事项','主要目标':'主要目标/重要事项',
    },
};
...
function getStructureForSheet(sheet, enabledIndex=-1){
    ...
    return structures.find(item => item.tableName === sheet.name) || structures.find(item => Number(item.tableIndex) === enabledIndex) || null;
}
...
function splitValueSheet(valueSheet, sheetName=''){
    ...
    const rows=valueSheet.slice(1).map(row=>{ const source=Array.isArray(row)?row:[]; return hasIndexColumn?source.slice(1):source.slice(); });
...
function currentSheetSnapshot(sheet){
    ...
    if(rawHeaders.length&&parsed.headers.length!==rawHeaders.length){
        const rows=valueSheet.slice(1).map(row=>{ const source=Array.isArray(row)?row:[]; return source.length===rawHeaders.length+1?source.slice(1):source.slice(0,rawHeaders.length); });
...
function rowMap(headers,row){
    const map=new Map();
    headers.forEach((header,index)=>{ if(!header)return; const value=row?.[index]??''; if(!map.has(header)||normalize(map.get(header))==='') map.set(header,value); });
    return map;
}
function findOldRow(sheetName,oldSnapshot,incomingHeaders,incomingRow){
    ...
    if(keys.length){
        ...
        if(usableKeys.length){
            const candidates=oldRows.filter(oldRow=>{ const old=rowMap(oldSnapshot.headers,oldRow); return usableKeys.every(key=>normalize(old.get(key))===normalize(incoming.get(key))); });
...
function conformValueSheetToSchema(sheet,valueSheet,enabledIndex=-1){
    ...
    const extraHeaders=(oldSnapshot.rawHeaders||oldSnapshot.headers).filter(header=>header&&!standardHeaders.includes(canonicalHeader(sheet.name,header)));
    ...
    const projectedRows=incoming.rows.map(row=>{
        ...
        return targetHeaders.map(header=>{ const canonical=canonicalHeader(sheet.name,header); if(incoming.headers.includes(canonical))return incomingValues.get(canonical)??''; return oldValues.get(canonical)??''; });
    });
    return [['',...targetHeaders],...projectedRows.map(row=>['',...row])];
}
function installWorldMemorySchemaGuard(sheet,enabledIndex=-1){
    ...
    sheet.rebuildHashSheetByValueSheet=function guardedRebuild(valueSheet,...args){ return original.call(this,conformValueSheetToSchema(this,valueSheet,enabledIndex),...args); };
...
function repairMissingColumnsBeforeCleanup({notify=true}={}){
    ...
    sheets.forEach((sheet,enabledIndex)=>{
        ...
        if(needsRepair){
            ...
            repaired.push({tableIndex:structure?.tableIndex??enabledIndex,tableName:sheet.name,missingHeaders,reordered:missingHeaders.length===0});
        }
    ...
    if(repaired.length>0){
        ...
        USER.saveChat();
        console.log('[Memo] 已统一七表标准表头:',repaired);
        if(notify){
            const summary=repaired.map(item=>item.missingHeaders.length?`${item.tableName}: 补齐/归一 ${item.missingHeaders.join('、')}`:`${item.tableName}: 已恢复标准顺序`).join('；');
