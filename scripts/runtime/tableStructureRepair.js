...
// 七张世界状态表的标准结构。标准列不可被AI整理/旧预设误改；真正的自定义附加列仍保留。
...
function getStructureForSheet(sheet, enabledIndex = -1) {
    ...
    return structures.find(item => item.tableName === sheet.name)
        || structures.find(item => Number(item.tableIndex) === enabledIndex)
        || null;
}
...
function repairMissingColumnsBeforeCleanup({ notify = true } = {}) {
    ...
    if (repaired.length > 0) {
        ...
        if (notify) {
            const summary = repaired.map(item => item.missingHeaders.length
                ? `${item.tableName}: 补齐/归一 ${item.missingHeaders.join('、')}`
                : `${item.tableName}: 已恢复标准顺序`).join('；');
