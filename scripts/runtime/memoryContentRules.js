...
function fixPersonSchema(settings) {
    ...
    person.note = '值得长期记忆的NPC；同一人物一行。A类当前状态字段覆盖更新；B类长期信息字段合并更新。姓名为正式主称呼，昵称/外号/道号/稳定职衔写入别名/称呼';
    person.initNode = '只记录后续值得继续引用的NPC；身份未明可暂用描述性称呼作姓名';
    person.insertNode = '出现新的重要NPC且表中没有时插入；严格按既定9列表头写入，不得把表头文字作为数据行';
    person.updateNode = 'A类：姓名/身份所属/修为/与玩家关系/当前状态按最新已确认值覆盖；B类：别名称呼/外貌特征/性格/重要信息合并仍有效内容；正式姓名出现时替换临时描述性姓名';
}
...
console.log('[世界状态记忆表格] 人物表A/B更新规则、角色身份归一、空值与神识术语规则已加载');
